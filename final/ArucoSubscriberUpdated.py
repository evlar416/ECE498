...
class ArucoSubscriber(Node):
    ...
    def listener_callback(self,aruco_msg):
        ...
        for i in range(len(aruco_msg.markers)):
            ...
            r = r.as_rotvec()
            # update orientation of marker in dictionary
            self.markerOrientation.update({i:[r[0],r[1],r[2]]})
            #self.markers[i] = [r[0],r[1],r[2]]
            # update position of maker in dictionary
            x = aruco_msg.markers[i].pose.position.x
            y = aruco_msg.markers[i].pose.position.y
            z = aruco_msg.markers[i].pose.position.z
            self.markerPosition.update({i:[x,y,z]})
...
def main(args=None):
    ...
    goal_marker = 0
    endpos=[aruco_subscriber.markerPosition[goal_marker][0],aruco_subscriber.markers[goal_marker][2]]
    
    # establish all obstacles (other identified markers)
    # obstacles = [(0,0) for i in range(len(aruco_subscriber.markerPosition))]
    obstacles = []

    for marker in aruco_subscriber.markerPosition.keys():
        if marker == goal_marker:
            continue
        else:
            obstacles.append((aruco_subscriber.markerPosition[marker][0],aruco_subscriber.markerPosition[marker][2]))
    ...
    goal_state = state_space(aruco_subscriber.markers[goal_marker][0],
                             aruco_subscriber.markers[goal_marker][1],
                             aruco_subscriber.markers[goal_marker][2])
